...
def scrape_snapdeal_data(max_pages=1):
    ...
    while page <= max_pages:
        url = f"https://www.snapdeal.com/products/beauty?sort=plrty"
        r = requests.get(url)
        ...
        for product in products:
            ...
            #description = product.find('div', class_='product-desc')
            ...
            # Create a tuple for each item (id, name, price, original_price, discount, ratings, image, product_link, description)
...
data_to_insert = scrape_snapdeal_data(max_pages=1)  # Set the number of pages to scrape
